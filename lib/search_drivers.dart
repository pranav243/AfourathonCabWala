...
class _SearchDriversState extends State<SearchDrivers> {
  ...
  searchResultList(){
    ...
    if(_searchController.text !="")
    {
      for(var snapShot in _allResults)
      {
        ...
        if(driverId.contains(_searchController.text.toLowerCase()) || name.contains(_searchController.text.toLowerCase()) || contact.contains(_searchController.text.toLowerCase()) || location.contains(_searchController.text.toLowerCase()))
        {
  ...
  getCollectionStream() async {
    var data = await FirebaseFirestore.instance.collection('Drivers').orderBy('Driver ID').get();
